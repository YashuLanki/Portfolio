...
|---|---|---|---|---|
| Vaulter AI Property Intelligence System | `YashuLanki/Vaulter_AI` | Python · MCP · Claude Desktop · SQLite | — (internal tool) | The firm's umbrella system: filename-based search over ~493k shared documents, plus a built-in CoStar screener and proximity tool — combining the two projects below into one, exposed as MCP tools through each teammate's own Claude Desktop. **Corrected 2026-07-29:** earlier drafts said "RAG-backed" search; the repo's own README confirms the ChromaDB/vector approach was removed in a July 2026 rebuild in favor of filename/path search over the OneDrive-synced library. |
| CoStar Listing Screener | `YashuLanki/Costar_screener` | Python · Claude API · Google Maps | — | Standalone, more elaborate version of Vaulter AI's screening idea: 4-phase pipeline — hard-rule elimination → composite ranking → Claude qualitative analysis → Google Maps ground-truth verification. Real funnel: 216 raw listings → 5 verified candidates. **Corrected 2026-07-29:** earlier draft said 9 candidates; the README confirms the final tier is 5. |
| Proximity Mapper | `YashuLanki/Proximity-Intel` | Python · Google Places API · GeoJSON | — | Standalone version of the proximity tool built into Vaulter AI: given a property, finds nearby employers/anchors across 17 categories, computes distance & direction, exports GeoJSON/CSV for due diligence. |
